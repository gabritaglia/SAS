/*iniziamo con una funzione interessante che abbiamo sorvolato la scorsa volta per mancanza di tempo*/
...
/*ora una procedura per rimodulare il dataset*/
...
/*come vi avevo anticipato la scorsa volta vediamo cosa sono gli arrey*/
...
/* RAPPRESNTAZIONI MAPPE */

libname mappe '\\tsclient\ste_sas\mappe_sas'; run;
options ls=256 ps=90 /*PAGESIZE=90*/ pageno=1 nocenter nodate;


/*comuni al 31-12-2010*/
PROC MAPIMPORT 
DATAFILE='\\vmware-host\Shared Folders\Desktop\ste_sas\mappe_sas\comuni2010\com2010_s.shp' /* file di input */
OUT=mitacom10; 
RUN;

/* importo dataset anagrafica*/
proc import  datafile="\\vmware-host\Shared Folders\Desktop\SAS_lezioni_master\Lezione_1\dataset_lez1\dataset_health.xls"
		dbms=excel 
		out=dataset_health2
		replace;
     getnames=yes;
run;
/*sistemo formati, nomi variabili e upcases*/
data dataset_health2;
set dataset_health2;
format nome_comune $50.;
rename comune=nome_comune;
run;
data Mitacom10;
set Mitacom10;
format nome_comune $50.;
nome_comune=upcase(nome_com);
run;
proc sort data=mitacom10;
by nome_comune;
run;
/*per fare serve una tabella con nome_comune e codici --> con la means prendo solo le medie*/
/*perchè le medie? le medie di valori tutti uguali sono i valori stessi!*/
proc means data=mitacom10 n mean;
by nome_comune;
output out=means;
RUN;
data means;
...
keep nome_comune cod_com cod_pro cod_reg objectid;
run;
/*ora estraggo le asl*/
data dataset_asl;
set dataset_health2;
if tipologia^="AZIENDA OSPEDALIERA" then delete;
run;

/*voglio rappresentare le frequenze, le calcolo*/
proc freq data=dataset_health2;
tables nome_comune/ out=FreqAO sparse;
run;
data FreqAO;
set FreqAO;
rename COUNT=aziende_ospedaliere;
drop PERCENT;
run;

proc sort data=mitacom10; by nome_comune;  run; 
proc sort data=FreqAO; by nome_comune;  run; 
data A_O; merge mitacom10 FreqAO ; by nome_comune; run;

/*Select Lombardy only*/               
data A_O_lomb;
set A_O;
if COD_REG^=3 then delete;
run;
/*mappa della Lombardia solamente, taglio altre regioni*/
data mitacom10_lomb;
set mitacom10;
if COD_REG^=3 then delete;
run;
...
/*
black blue brown cyan gray green lime magenta orange pink red white
yellow
potete schiarirli (es. lightblue) o renderli più scuri (es. darkred)
*/

FILENAME mappa "\\vmware-host\Shared Folders\Desktop\mappe_sas\mappa.png";
goptions reset=(axis, legend, pattern, symbol, title, footnote) norotate
          hpos=0 vpos=0 htext= ftext= ctext= target= gaccess= gsfmode= ;
 goptions device=WIN  ctext=blue CBACK=WHITE
  ...
  *------------------------------------------------------------------*/
 pattern1 color=lightRED value=MSOLID;
 pattern2 color=lightBLUE value=MSOLID;
 pattern3 color=lightYELLOW value=MSOLID;
 pattern4 color=lightCYAN value=MSOLID;
 pattern5 color=lightGRAY value=MSOLID;
 pattern6 color=PINK value=MSOLID;
 pattern7 color=WHITE value=MSOLID;
 pattern8 color=GREEN value=MSOLID;
 pattern9 color=MAGENTA value=MSOLID;
 pattern10 color=ORANGE value=MSOLID;
 pattern1 value=MSOLID;
   proc gmap map=mitacom10_lomb data=A_O_lomb ;
   id cod_com;
LABEL aziende_ospedaliere='aziende_ospedaliere';
CHORO aziende_ospedaliere
    /
    levels=50  /* midpoints=10 to 100 by 20*/ MISSING  /*anno=anno_italy*/ coutline=greybb /*same*/
    ;
 run;
